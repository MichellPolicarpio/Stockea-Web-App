            ...
            {buildings.length === 0 ? (
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {buildings.map((building) => (
                        <Card key={building.id} className="cursor-pointer hover:shadow-md transition-shadow overflow-hidden group">
                            {/* Imagen de Preview */}
                            <div className="h-48 w-full overflow-hidden bg-slate-100 relative">
                                ...
                                {/* Overlay gradiente para texto legible si se quisiera poner texto encima, pero aqui es visual */}
                                <div className="absolute inset-0 bg-gradient-to-t from-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                            </div>

                            <CardHeader>
                                <CardTitle className="flex items-center gap-2">
                                    {/* Icono pequeño si no hay imagen, pero lo dejo por consistencia */}
